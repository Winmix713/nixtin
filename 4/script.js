...
async function loadHistoricalData() {
    try {
        ...
        historicalData = data.map(match => ({
            ...
            both_teams_scored: match.both_teams_scored === true
        }));
        
        const modelInfo = document.getElementById('modelInfo');
        modelInfo.textContent = 'Adatok betöltve, a rendszer készen áll a predikciókra!';
        modelInfo.classList.remove('hidden');
    } catch (error) {
        console.error('Hiba történt az adatok betöltése során:', error);
        const modelInfo = document.getElementById('modelInfo');
        modelInfo.textContent = 'Hiba történt az adatok betöltése során. Kérjük, próbálja újra később.';
        modelInfo.classList.remove('hidden');
    }
}
...
function runPredictions() {
    ...
    if (matchups.length === 0) {
        alert('Kérjük, adjon hozzá mérkőzéseket az előrejelzéshez!');
        return;
    }
...
function predictMatch(match) {
    ...
    if (totalMatches === 0) {
        return {
            ...
            noData: true,
            explanation: `
                Nincs elegendő korábbi mérkőzés adat az előrejelzéshez a ${match.homeTeam} és ${match.awayTeam} mérkőzésre.
            `
        };
    }
